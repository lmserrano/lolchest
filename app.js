...
// NOTE: TEMP DEBUG - Don't commit and push this
var api_key = '???';
var port = '8080';
...
function getSummonerId(params, res) {
    ...
    var region = regions[params.region].toLowerCase();
    var url = 'https://'+region+'.api.riotgames.com/lol/summoner/v3/summoners/by-name/'+summoner_name+'?api_key='+api_key;
    ...
    (error) => {
        console.log("Error on getSummonerId for params: " + JSON.stringify(params));
        res.json(error);
    });
}
...
function getChampionMastery(data, res) {
    ...
    var url = 'https://'+region+'.api.riotgames.com/lol/champion-mastery/v3/champion-masteries/by-summoner/'+data['summoner_id']+'?api_key='+api_key;
    makeRequest(url,
        (mastery_info) => {
    ...
    (error) => {
        console.log("Error on getChampionMastery for data: " + JSON.stringify(data));
        res.json(error);
    });
}
